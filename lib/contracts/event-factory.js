    ...
    return new _promise2.default(function (resolve, reject) {
      ...
      while (eventIndex < contractData.length) {
        ...
        if (events[eventHash].kind === constants.KIND_RANGED) {
          if (contractData[eventIndex + 3].greaterThan(new _bignumber2.default(2).pow(128))) {
            events[eventHash].lowerBound = contractData[eventIndex + 3].minus(new _bignumber2.default(2).pow(256));
          } else {
            events[eventHash].lowerBound = contractData[eventIndex + 3];
          }

          if (contractData[eventIndex + 4].greaterThan(new _bignumber2.default(2).pow(128))) {
            events[eventHash].upperBound = contractData[eventIndex + 4].minus(new _bignumber2.default(2).pow(256));
          } else {
